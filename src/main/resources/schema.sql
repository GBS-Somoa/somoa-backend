...
CREATE TABLE `Device_Supply` (
                                 `device_id`    VARCHAR(36)    NOT NULL    COMMENT 'UUID',
                                 `supply_id`    INT    NOT NULL    COMMENT 'Supply 컬렉션과 1:N',
                                 `created_at`    TIMESTAMP    NOT NULL,
                                 `updated_at`    TIMESTAMP    NOT NULL
);
...
ALTER TABLE `Group_User` ADD CONSTRAINT `PK_GROUP_USER` PRIMARY KEY (
                                                                     `user_id`,
                                                                     `group_id`
    );
...
ALTER TABLE `Device_Supply` ADD CONSTRAINT `PK_DEVICE_SUPPLY` PRIMARY KEY (
                                                                           `device_id`
    );
